...
public class UserDBManager extends RedisDBManager {
    ...
    public boolean expireSession(String sessionID) {
        ...
        if (name != null) {
            ...
            if (jedis.exists(getKeyOfBrowsingHistory(sessionID))) {
                jedis.expire(getKeyOfBrowsingHistory(sessionID), 1);
            }
            // remove shopping cart : use TTL
            if (jedis.exists(getKeyOfShoppingCart(sessionID))) {
                jedis.expire(getKeyOfShoppingCart(sessionID), 1);
            }
            // make the value at offset as '0'
            jedis.setbit(getKeyOfSessionIdTracker(), sessionValue, false);
            returnConnection(jedis);
            return true;
        }
        returnConnection(jedis);
